from pyogg.opus import ctypes

from pyogg import opus
from pyogg import PyOggError


class OpusBufferStream:
    def __init__(self, pcm):
        ...
        self.of = opus.op_open_memory(
            pcm_ctypes, len(pcm_ctypes), ctypes.pointer(error))

        if error.value != 0:
            self.of = None
            raise PyOggError(
                "file couldn't be opened or doesn't exist. Error code : {}".format(error.value))
    ...
    def get_buffer(self):
        ...
        if samples_read < 0:
            raise PyOggError(
                "Failed to read OpusFileStream.  Error {:d}".format(
                    samples_read)
            )
